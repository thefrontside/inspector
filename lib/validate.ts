...
export function validateUnsafe<T>(schema: StandardSchemaV1<T>, value: unknown): StandardSchemaV1.InferInput<StandardSchemaV1<T>> {
  return unbox(validate(schema, value));
}

export function validate<T>(schema: StandardSchemaV1<T>, value: unknown): Result<StandardSchemaV1.InferInput<StandardSchemaV1<T>>> {
