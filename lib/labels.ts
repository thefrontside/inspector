...
export const LabelsContext = createContext<Labels>("@effectionx/inspector.labels", { name: "anonymous" });

export function* useLabels(labels: Labels): Operation<Labels> {
  let scope = yield* useScope();

  if (!scope.hasOwn(LabelsContext)) {
    return yield* LabelsContext.set({ ...LabelsContext.defaultValue, ...labels });
  } else {
