...
import { createChannel, Stream, useAbortSignal } from "starfx";
        ...
        yield* spawn(function* () {
          ...
          while (!next.done) {
            ...
            if (event === "progress") {
              ...
              validateUnsafe(protocol.methods[name].progress, parsed);
              yield* channel.send(parsed);
              next = yield* subscription.next();
              console.log({ next });
            } else if (event === "return") {
              console.log("return event", { data });
              validateUnsafe(protocol.methods[name].returns, parsed);
              yield* channel.close(parsed);
