...
export function Graphic({ hierarchy }: { hierarchy?: Hierarchy }) {
  ...
  return hierarchy ? (
    <div id="treeWrapper" style={{ width: "100vh", height: "1024px" }}>
      <Tree data={transform2D3(hierarchy)} />
    </div>
  ) : <div></div>;
}
...
function transform2D3(hierarchy: Hierarchy): RawNodeDatum {
  let { data } = hierarchy;
  let { name, ...attributes } = (data[LabelsContext.name] ?? LabelsContext.defaultValue) as Labels;
  return {
    name: name as string,
